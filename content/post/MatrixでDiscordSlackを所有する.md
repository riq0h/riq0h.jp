...
そこで、ようやく手を出したのが[Matrix](https://matrix.org)だった。情報技術系のコミュニティで広く使われていたり、やたら熱心な信奉者を見かけるこのプロトコルは、分散型ネットワークを形成するためセルフホスト型でありながら一つのアカウントで他のサーバに接続することができる。（プロトコルの発展次第で）作ったアカウントが活かせるのは説得材料になりうる。
